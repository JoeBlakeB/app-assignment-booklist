...
const settings = {
    ...
    openSettingsPage: function () {
        ...
        // Search options
        settingsHTML += "<h2 class='searchSettings'>Search</h2>\
            <div class='searchSettings'><label class='searchShowTime' for='searchShowTime'><input class='searchInput' onchange='settings.searchShowTimeChange(this);' type='checkbox' id='searchShowTime' name='searchShowTime'";
        if (this.searchShowTime) {
            settingsHTML += " checked";
        }
        settingsHTML += "><h3>Show search results response time</h3></label>\
            <br /><label class='searchBooksPerPage' for='searchBooksPerPage'><select id='searchBooksPerPage' name='searchBooksPerPage' onchange='settings.searchBooksPerPageChange(this);' class='searchInput'>"
        for (let number of [10, 25, 50, 75, 100]) {
            settingsHTML += "<option value='" + number + "'";
            if (number == this.searchBooksPerPage) {
    ...
    searchShowTimeChange: function (object) {
        settings.searchShowTime = object.checked;
        document.cookie = "searchShowTime=" + object.checked + ";" + this.cookieSettings;
    },
    // Books per page drop down selected
    searchBooksPerPageChange: function (object) {
        settings.searchBooksPerPage = object.value;
        document.cookie = "searchBooksPerPage=" + object.value + ";" + this.cookieSettings;
    },
    // Settings for how the search looks
    searchShowTime: false,
    searchBooksPerPage: 25,
    ...
    getCurrentSettings: function () {
        ...
        for (let cookie of cookies) {
            ...
            switch (cookie[0]) {
                case "searchShowTime":
                    settings.searchShowTime = cookie[1] === "true";
                    break;
                case "searchBooksPerPage":
                    settings.searchBooksPerPage = cookie[1];
                    break;
            }
        }
    }
...
// Get settings then blank search on page load
